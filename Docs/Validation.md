...
The 182-case Swift suite passed locally and on hosted macOS 26 CI. All 218 Python checks passed, including the prepared-guest importer and release automation. The corrected factory `0.4.0`, integration version `4`, passed startup, adaptive-resolution, configuration-reload recovery, and SSH smoke checks. Native Home, TextEdit, and actual guest 512 × 320 and 1440 × 900 menu checks passed. Developer ID signed build `0.1.0 (10)` completed archive, export, packaging, and app/DMG notarization with the corrected factory verified in its outputs. Its changes after `fcb89b2` are limited to the teal icon and matching About colors. Hosted macOS 26 CI also passed for `d72e5a0`, including all 182 Swift cases and the Python checks. [GitHub Actions run 34638816093](https://github.com/Aayush9029/Omabox/actions/runs/34638816093). The README screenshots still show build 5. Results from preceding revisions are identified separately below.

## Release build

The completed Developer ID signed build `0.1.0 (10)` passes strict signature and release checks. It contains only arm64 code, targets macOS 26, and has App Sandbox, Hardened Runtime, and a secure signing timestamp. The app has virtualization, outbound networking, microphone input, and user-selected file/bookmark entitlements. It does not have debugger attachment, JIT, unsigned executable memory, library validation exceptions, or bridged networking entitlements. Debugger attachment remains available for Debug builds and tests.

The archive is `build/Archives/Omabox-0.1.0-10.xcarchive`; the exported app is `build/Export/Omabox-0.1.0-10/Omabox.app`. Both contain the corrected factory `0.4.0`, integration version `4`, with disk SHA-256 `8b2a11ab8efb6ab038e963b7f56244e59d2045b3eec6e8dfd802137416f0b30a`. The signing identity is Developer ID Application: Aayush Pokharel (`4538W4A79B`). Archive and export logs are `/tmp/omabox-production-archive-build10.05CDfw` and `/tmp/omabox-production-export-build10.6xgaON`. The compiled teal ICNS was visually verified and matched between the archive and exported app. Apple accepted both the app (`3d2169c3-7948-40e4-951c-16acb005d536`) and DMG (`739017fb-8a9c-4404-8fa6-bae573f94bfe`). Both stapled tickets, strict signatures, Gatekeeper assessments, disk-image integrity, and read-only mounted app/license checks passed. The final `Omabox-0.1.0-arm64.dmg` is 1,752,762,739 bytes with SHA-256 `52d89d426632c2f957631075734470f3ca1f44bd6d0b72a4114b778dd0d75d90`. Its release manifest records source commit `d72e5a0` and CI run `34638816093`. Installation on a second Mac has not been performed.
...
The latest local run used a fresh Debug build-for-testing and loaded its app library and test bundle through Swift Testing's stable tools ABI. A minimal AppKit context with activation prohibited supported hidden-window geometry, real SwiftUI hosting, and immediate-focus regressions. It did not launch Omabox's application entry point, start a VM, or require the stalled Xcode test daemon. Evidence, tested binary hashes, raw events, exact invocation, and counted completions are in `Artifacts/Overnight/UI/animation-config-regressions/`; the process exited 0. The runner methodology is documented in `Artifacts/Overnight/UI/UnitTesting.md`. Earlier runtime/SSH, palette-focus, and quit/resume runs remain in their artifact directories, and the 39-case Xcode run is preserved separately in `Artifacts/Validation/unit-results.json`.

Hosted macOS 26 CI passed for commit `fcb89b2`: 124 Swift test functions across 15 suites, covering 182 cases. [GitHub Actions run 34637358288](https://github.com/Aayush9029/Omabox/actions/runs/34637358288) completed successfully; the retrieved log is `/tmp/omabox-fcb89b2-native-ci.log`. This verifies the native test suite on the deployment-target operating system, not a live guest session or automated UI run.

All 218 Python checks passed: 82 guest-service tests, 16 clipboard-service tests, and 120 script tests. The script suite includes 24 release-verifier checks, 47 prepared-guest importer checks, and 49 release-automation checks. The combined `Scripts/ci.sh guest` run exited 0. Coverage includes configuration reloads, corrupted downloads, source drift, exclusive guest publication, exact-commit CI gates, interrupted release uploads, stale drafts, and published-release immutability.
...
The README contains exactly two screenshots captured from Developer ID signed build 5, `Docs/images/home.png` and `Docs/images/settings.png`. Each is an opaque 2240 × 1440 PNG showing the actual desktop wallpaper on the same 1120 × 720-point canvas. Home is centered at 880 × 560 points and Settings at 720 × 500 points. Other windows were hidden, and Home was minimized for the Settings capture.
...
Both GitHub workflows passed `actionlint` 1.7.12, and the release credential helper passed shell syntax validation. A read-only check against the actual GitHub API selected `d72e5a0` and its successful CI run correctly. The finalized DMG passed an actual prepared-guest import in an isolated checkout, including payload verification, atomic publication, and clean volume detachment; the report is `Artifacts/prepared-guest-import-ylocqz_n/import-report.json`. End-to-end release results are reported by the [Release workflow](https://github.com/Aayush9029/Omabox/actions/workflows/release.yml).

Physical microphone recording, guest audio playback to a physical output device, browser screen-share selection, non-US keyboard layouts, external display changes, host sleep/wake, and live guest execution on macOS 26 remain unverified. Linux graphics use llvmpipe software rendering through Apple's Virtio framebuffer; no Linux GPU acceleration is claimed.
